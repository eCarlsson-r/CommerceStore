    ...
    <section className="mb-8 rounded-2xl border border-gray-200 p-4">
      ...
      <div className="flex flex-row md:flex-col gap-3">
        <input
          ...
          placeholder={t('pasteRoomPhotoUrl')}
        ...
        >
          {visualSearch.isPending ? t('searching') : t('findSimilar')}
        </Button>
        <Button
          variant="outline"
          onClick={() => {
      ...
      {renderInlineResults && !!visualSearch.data?.items.length && (
        <div className="mt-4 space-y-3">
          <p className="text-sm text-gray-700">{t('visualMatchesTitle')}</p>
          {isLoadingProducts ? (
            <p className="text-sm text-gray-500">{t('loadingMatchedProducts')}</p>
          ) : (
